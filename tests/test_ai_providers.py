...
def test_environment_factory_supports_mixed_capabilities(
    ...
    monkeypatch.setenv("LAFVIN_LLM_API_KEY", "llm-secret")
...
def test_openai_alias_supports_all_capabilities(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.parametrize(
    ("provider_name", "key_name", "base_url"),
    [
...
def test_openai_compatible_llm_presets_set_service_defaults(
    ...
    monkeypatch.setenv("LAFVIN_LLM_MODEL", "provider-model")
...
def test_claude_uses_native_anthropic_provider(
    ...
    monkeypatch.setenv("LAFVIN_LLM_MODEL", "claude-model")
...
def test_named_tts_providers_use_service_defaults(
    ...
    monkeypatch.setenv(key_name, "provider-secret")
    monkeypatch.delenv("LAFVIN_TTS_MODEL", raising=False)
    monkeypatch.delenv("LAFVIN_TTS_VOICE", raising=False)
...
def test_fish_audio_asr_uses_service_defaults(
    ...
    monkeypatch.setenv("LAFVIN_ASR_MODEL", "transcribe-1")
    monkeypatch.setenv("LAFVIN_ASR_LANGUAGE", "zh")
...
def test_named_llm_provider_requires_model(
    ...
    monkeypatch.delenv("LAFVIN_LLM_MODEL", raising=False)

    with pytest.raises(AIConfigurationError, match="LAFVIN_LLM_MODEL"):
        providers_from_environment(require_explicit=True)
...
def test_explicit_configuration_requires_every_capability(
    ...
    monkeypatch.delenv("LAFVIN_AI_PROVIDER", raising=False)
...
def test_explicit_custom_llm_configuration_requires_api_key(
    ...
    monkeypatch.setenv("LAFVIN_TTS_PROVIDER", "fake")
    monkeypatch.setenv("LAFVIN_LLM_BASE_URL", "https://llm.invalid/v1")
    monkeypatch.delenv("LAFVIN_LLM_API_KEY", raising=False)

    with pytest.raises(AIConfigurationError, match="LAFVIN_LLM_API_KEY"):
        providers_from_environment(require_explicit=True)
...
def test_custom_llm_requires_an_explicit_base_url(
    ...
    monkeypatch.setenv("LAFVIN_LLM_PROVIDER", "openai-compatible")
    monkeypatch.setenv("LAFVIN_TTS_PROVIDER", "fake")
    monkeypatch.setenv("LAFVIN_LLM_API_KEY", "secret")
    monkeypatch.delenv("LAFVIN_LLM_BASE_URL", raising=False)

    with pytest.raises(AIConfigurationError, match="LAFVIN_LLM_BASE_URL"):
        providers_from_environment(require_explicit=True)
...
def test_legacy_common_fake_configuration_remains_available(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
